...
export default function App() {
  ...
  const [intelSection, setIntelSection] = useState("tendencies");
  ...
  return (
    <div className="p-3 bg-slate-900 min-h-screen text-white text-sm">
      ...
      {view === "intel" && (
        <div>
          <div className="flex gap-1 mb-3 justify-center">
            {[{k:"tendencies",l:"Tendencies"},{k:"risk",l:"Risk Index"},{k:"clutch",l:"Clutch Splits"}].map(s => (
          ...
          {intelSection === "tendencies" && (
            <>
          <div className="bg-slate-800 rounded-lg p-3 mb-3">
            <h2 className="font-bold text-yellow-400 mb-2">Player Tendencies</h2>
            <table className="w-full text-xs">
              <thead>
                <tr className="text-gray-400 border-b border-slate-600">
                  ...
                  <th className="p-1">Poss</th>
                  <th className="p-1">PPP</th>
                  <th className="p-1">Rim%</th>
                  <th className="p-1">Mid%</th>
                  <th className="p-1">3PT%</th>
                  <th className="p-1">Self%</th>
                  <th className="p-1">Ast'd%</th>
                </tr>
              </thead>
              <tbody>
                {playerTendencies.sort((a, b) => b.totalPoss - a.totalPoss).map(p => (
                  <tr key={p.name} className="border-b border-slate-700">
                    ...
                    <td className={`p-1 text-center font-bold ${getPPPColor(p.ppp)}`}>{p.ppp.toFixed(2)}</td>
                    <td className="p-1 text-center text-orange-400">{p.rimPct.toFixed(0)}%</td>
                    <td className="p-1 text-center text-yellow-400">{p.midPct.toFixed(0)}%</td>
                    <td className="p-1 text-center text-cyan-400">{p.threePct.toFixed(0)}%</td>
                    <td className="p-1 text-center text-purple-400">{p.selfCreatedPct.toFixed(0)}%</td>
                    <td className="p-1 text-center text-green-400">{p.assistedPct.toFixed(0)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-slate-800 rounded-lg p-3 mb-3">
            <h3 className="font-semibold text-sm mb-2">Shot Diet</h3>
            {playerTendencies.sort((a, b) => b.totalPoss - a.totalPoss).slice(0, 8).map(p => (
              <div key={p.name} className="bg-slate-700 rounded p-2 mb-2">
                <div className="flex justify-between mb-1">
                  <span className="font-medium text-xs">{getDisplayName(p.name)}</span>
                  <span className="text-xs text-gray-400">{p.totalPoss} poss</span>
                </div>
                <div className="flex h-3 rounded overflow-hidden">
                  <div className="bg-orange-500" style={{width: `${p.rimPct}%`}}></div>
                  <div className="bg-yellow-500" style={{width: `${p.midPct}%`}}></div>
                  <div className="bg-cyan-500" style={{width: `${p.threePct}%`}}></div>
                </div>
                <div className="flex justify-between text-xs mt-1 text-gray-500">
                  <span>Rim {p.rimPct.toFixed(0)}%</span>
                  <span>Mid {p.midPct.toFixed(0)}%</span>
                  <span>3PT {p.threePct.toFixed(0)}%</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-slate-800 rounded-lg p-3">
            <h3 className="font-semibold text-sm mb-2">Creation Breakdown</h3>
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-purple-900/30 rounded p-2">
                <div className="text-purple-400 font-semibold text-xs mb-1">Self-Creators (40%+)</div>
                {playerTendencies.filter(p => p.selfCreatedPct >= 40 && p.totalPoss >= 50).map(p => (
                  <div key={p.name} className="text-xs flex justify-between">
                    <span>{getDisplayName(p.name)}</span>
                    <span>{p.selfCreatedPct.toFixed(0)}%</span>
                  </div>
                ))}
              </div>
              <div className="bg-green-900/30 rounded p-2">
                <div className="text-green-400 font-semibold text-xs mb-1">Assisted (60%+)</div>
                {playerTendencies.filter(p => p.assistedPct >= 60 && p.totalPoss >= 50).map(p => (
                  <div key={p.name} className="text-xs flex justify-between">
                    <span>{getDisplayName(p.name)}</span>
                    <span>{p.assistedPct.toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {view === "playbook" && (
        <div>
          <div className="flex gap-1 mb-3 justify-center flex-wrap">
            {[{k:"playtypes",l:"Play Types"},{k:"winlose",l:"Win/Lose"},{k:"failures",l:"Failures"},{k:"theories",l:"Theories"}].map(s => (
        ...
        <div className="space-y-3">
          ...
          {playbookSection === "winlose" && (
        <div>
          <div className="bg-slate-800 rounded-lg p-3 mb-3">
            <h2 className="font-bold text-emerald-400 mb-3">✅ How Louisville Wins</h2>
            <div className="space-y-2">
              <div className="bg-emerald-900/20 rounded p-2 border-l-4 border-emerald-500">
                <div className="font-semibold text-emerald-400 text-sm">1. Shoot from Spot Up</div>
                <div className="text-xs text-gray-300">321p at 1.30 PPP. McKneely 51%, Hadley 52%, Brown 50%, Wooley 50% from 3. Get open looks.</div>
              </div>
              <div className="bg-emerald-900/20 rounded p-2 border-l-4 border-emerald-500">
                <div className="font-semibold text-emerald-400 text-sm">2. Feed Fru at Rim</div>
                <div className="text-xs text-gray-300">Cut (1.64 PPP, 89% FG), Roll (1.28 PPP), Putback (1.31 PPP). Walking bucket inside.</div>
              </div>
              <div className="bg-emerald-900/20 rounded p-2 border-l-4 border-emerald-500">
                <div className="font-semibold text-emerald-400 text-sm">3. Hadley Leads Transition</div>
                <div className="text-xs text-gray-300">38p at 1.26 PPP, 65% FG. Best fast break player by far.</div>
              </div>
              <div className="bg-emerald-900/20 rounded p-2 border-l-4 border-emerald-500">
                <div className="font-semibold text-emerald-400 text-sm">4. Hadley ISO Late Clock</div>
                <div className="text-xs text-gray-300">1.60 PPP on ISO (71% FG). Better than forced Pick & Roll.</div>
              </div>
              <div className="bg-emerald-900/20 rounded p-2 border-l-4 border-emerald-500">
                <div className="font-semibold text-emerald-400 text-sm">5. Elite Cutting</div>
                <div className="text-xs text-gray-300">Team 1.49 PPP. Fru 89%, Wooley 85%, Rooths 70% FG on cuts.</div>
              </div>
            </div>
          </div>

          <div className="bg-slate-800 rounded-lg p-3 mb-3">
            <h2 className="font-bold text-red-400 mb-3">❌ How Louisville Loses</h2>
            <div className="space-y-2">
              <div className="bg-red-900/20 rounded p-2 border-l-4 border-red-500">
                <div className="font-semibold text-red-400 text-sm">1. Wooley Creates</div>
                <div className="text-xs text-gray-300">P&R: 0.52 PPP (31p, 18% FG, 6 TO). Transition: 0.83 PPP (47p, 0% 3PT).</div>
              </div>
              <div className="bg-red-900/20 rounded p-2 border-l-4 border-red-500">
                <div className="font-semibold text-red-400 text-sm">2. P&R Turnovers</div>
                <div className="text-xs text-gray-300">49 TO on 203 poss (24% rate). Brown 10 TO, Conwell 13 TO.</div>
              </div>
              <div className="bg-red-900/20 rounded p-2 border-l-4 border-red-500">
                <div className="font-semibold text-red-400 text-sm">3. Rooths Shoots 3s</div>
                <div className="text-xs text-gray-300">0.65 PPP on 31 spot ups (16% 3PT). Only non-shooter.</div>
              </div>
              <div className="bg-red-900/20 rounded p-2 border-l-4 border-red-500">
                <div className="font-semibold text-red-400 text-sm">4. Brown Off-Ball</div>
                <div className="text-xs text-gray-300">Handoff 0.35 PPP (20p), Off Screen 0.27 PPP (15p). Disaster.</div>
              </div>
              <div className="bg-red-900/20 rounded p-2 border-l-4 border-red-500">
                <div className="font-semibold text-red-400 text-sm">5. Wrong Guys Lead Breaks</div>
                <div className="text-xs text-gray-300">Wooley/McKneely/Conwell: 162p at 0.90 PPP in transition.</div>
              </div>
              <div className="bg-red-900/20 rounded p-2 border-l-4 border-red-500">
                <div className="font-semibold text-red-400 text-sm">6. Post Up Non-Hadley</div>
                <div className="text-xs text-gray-300">Team without Hadley: 0.57 PPP (30p). Only he can post.</div>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          {playbookSection === "theories" && (
        <div>
          <div className="bg-slate-800 rounded-lg p-3 mb-3">
            <h2 className="font-bold text-yellow-400 mb-2">💡 Actionable Theories</h2>
            <p className="text-gray-400 text-xs mb-3">Based on high-volume trends (20+ possessions)</p>
            
            <div className="space-y-2">
              <div className="bg-emerald-900/30 rounded p-2 border-l-4 border-emerald-500">
                <div className="font-semibold text-emerald-400 text-sm">1. Spot Up First Offense</div>
                <div className="text-xs text-gray-300">321p at 1.30 PPP. McKneely (1.52), Hadley (1.51), Conwell (1.32), Wooley (1.29) all elite. Build around open 3s.</div>
              </div>

              <div className="bg-orange-900/30 rounded p-2 border-l-4 border-orange-500">
                <div className="font-semibold text-orange-400 text-sm">2. Transition Selectivity</div>
                <div className="text-xs text-gray-300">313p at 0.98 PPP. Hadley (1.26) and Fru (1.25) efficient. Wooley (0.83) and McKneely (0.89) struggle. Let Hadley/Fru lead.</div>
              </div>

              <div className="bg-blue-900/30 rounded p-2 border-l-4 border-blue-500">
                <div className="font-semibold text-blue-400 text-sm">3. P&R Creates Kick-Outs</div>
                <div className="text-xs text-gray-300">P&R at 0.76 PPP is below average. Use to collapse defense, then kick to shooters (1.30 PPP spot ups).</div>
              </div>

              <div className="bg-red-900/30 rounded p-2 border-l-4 border-red-500">
                <div className="font-semibold text-red-400 text-sm">4. Wooley Off P&R</div>
                <div className="text-xs text-gray-300">0.52 PPP on 31 P&R poss (18% FG, 6 TO). But 1.92 cutting, 1.29 spot up. Keep him off-ball.</div>
              </div>

              <div className="bg-purple-900/30 rounded p-2 border-l-4 border-purple-500">
                <div className="font-semibold text-purple-400 text-sm">5. Rooths = Cut Only</div>
                <div className="text-xs text-gray-300">0.65 PPP on 31 spot ups (16% 3PT). But 1.50 cutting (24p, 70% FG). Don't spot him up.</div>
              </div>

              <div className="bg-cyan-900/30 rounded p-2 border-l-4 border-cyan-500">
                <div className="font-semibold text-cyan-400 text-sm">6. Hadley Does Everything</div>
                <div className="text-xs text-gray-300">Trans 1.26, Spot 1.51, Post 1.03, ISO 1.60. Most versatile - should touch ball often.</div>
              </div>
            </div>
          </div>

          <div className="bg-slate-800 rounded-lg p-3">
            <h3 className="font-semibold text-sm mb-2">📊 Team Identity</h3>
            <div className="text-xs bg-emerald-900/30 rounded p-2">
              Louisville is a <strong className="text-emerald-400">spot-up shooting team</strong> with elite 3PT shooters across the roster. Prioritize open 3s through P&R penetration and ball movement. Cutting is highly efficient but lower volume. Transition is mixed - lean on Hadley/Fru.
            </div>
          </div>
        </div>
          )}
